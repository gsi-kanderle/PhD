...
In the scope of our study, the DIR was included in the contour propagation, 4D optimization and 4D dose calculation. Contour propagation was done from a planning CT to a 4D-CT 
reference phase and from the 4D-CT reference phase to all other phases. 
from 


For calculation of DIR,  

% Furthermore, pig cardiac 4D-CTs had to be registered under time pressure,
% which had
The advantage of using open-source software for DIR, as explained in Section~\ref{RegistrationImplement}

a single registration algorithm, B-Spine, was used in lung and cardiac 4D-CT DIR, becaue it is better suited for large deformations present in lung and cardiac 4D-CT \cite{Tang2013}. 
In contrast, Demons algorithm is usually used for small deformations. Additionally, it was shown on a  that B-Spline preforms better than on a lung patient CT
The B-Spline
algorithm handles large deformation, whereas Demons 
B-Spline algorithm should handle large deformations well, which
...
The , is that different DIR algorithms, 
...
There are large deformations present in 
