...
to the reference phase, where the dose from all motion states is accumulated. 4D dose calculation requires accurate DIR at every voxel, since errors in DIR can significantly alter the 4D dose \cite{Heath2006}. Special consideration has to be paid to calculation of biological 4D doses \cite{Gemmel2011}. 
...
Phase 0 and 5 correspond to the end-inhale and end-exhale breathing state, respectively. Phase 0 was chosen as a reference phase. 23 4D-CTs of lung cancer patients were used in this study.

True and inverse DIR were computed for each patient between each phase and the reference phase. Each 4D-CT required 18 DIRs, leading to 414 DIRs in total.
...
\begin{figure}[H]
	\begin{center}		
		...
		Minimum ICE is 0 throughout all phases and patients. Boxes represent 25-75\%, whiskers 10-90\% of data, median is shown with a solid line, mean with squares and outliers with crosses.}
		\label{jacobian_data}
	\end{center}
\end{figure}
...
\begin{figure}[H]
	\begin{center}		
		...
		\caption{Values of maximum Jacobian (a), minimum Jacobian (b) and maximum ICE (c) plotted against maximum vector magnitudes. A linear fit is displayed with a solid line and parameters are written below the plots. Dashed line in (c) shows $y(x)= x$ plot.
			...
			The ICE is displayed using color table as displayed in legend.}
		\label{maxvf}
	\end{center}
\end{figure}
...
\begin{figure}[H]
	\begin{center}		
		...
		Minimum ICE is 0 throughout all phases and pigs. Boxes represent 25-75\%, whiskers 10-90\% of data, median is shown with a solid line, mean with squares and outliers with crosses.}
