...
Figure 5‑2. Explanation of the computation of the mean motion vector. Since all vectors have the same
starting point, vectors can be averaged resulting in the mean vector. To compute the vectors to the frames
with respect to the mean position, the mean vector needs to be subtracted (using a numerical inversion
step since the vector space is not a continuum) from the original vector to the frame with respect to the
reference (exhale) frame.
Image quality optimization | 85
to obtain the MidP CT scan, where all the internal structures, including the tumor,
were in their exact time-weighted mean position of the respiratory motion. Arithmetic
averaging (MidPmean) as well as median averaging (MidPmedian) were investigated.
