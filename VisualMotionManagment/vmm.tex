...
The changes in patient anatomy can be seen on CT (4DCT) or MRI scan. To asset this changes a registration must be preformed. Registration can be made between different imaging modalities, scans from different days or phases in 4DCT. Different algorithms and software is available
...
Here, $x$ and $x'$ are points in states $r$ and $i$, respectively and $u_{ri}$ is a vector field representation of the transformation map. $u_{ri}$ can among others be used for assessing motion amplitudes, contour propagation and 4D dose reconstruction. It is important to note
that for certain steps in 4D treatment planning require also inverse registration, from state $i$ to $r$ \cite{Richter2012}.

Plastimatch \cite{Shackleford2010} is a commonly used software for registration in medical research. It is free and open-source software, available as a command-line executable program. Plastimatch B-spline registration is also available in Slicer as an external module, SlicerRT \cite{Pinter2012}.  
...
In order to be able to provide a clear description of methods and procedures used, an overview of the expressions used will be given here.
...
and DIRQA in case if they are interrupted by Slicer crash. DIR and DIRQA files can be quite large and can cause Slicer to run out of memory. Patient hierarchy enables then to continue work
...
Absolute difference can be computed between reference and moving image, named default absolute difference, to provide initial estimate of difference between images. After the registration it can be done on
true and inverse warped images (true and inverse absolute difference) to show registration results. Usually, registration works on minimizing absolute difference, so it is a direct indicator
...
should be preformed separately In ideal scenario, $u_{AB}$ would be a direct inverse of $u_{BA}$. However, deformable image registration algorithms do not yield perfectly inverse consistent vector field.
