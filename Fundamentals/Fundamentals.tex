...
Ions used in radiotherapy loose most of their energy via inelastic Coulomb scattering on target electrons. Electrons liberated from target by ions are called secondary electrons or $\delta$-electrons. $\delta$-electrons travel through matter, scatter further and may produce secondary ionization of the target atoms. When $\delta$-electrons energies are larger than $>$50 eV,
...
The aim of radiotherapy is to kill tumor cells and prevent further growth, while sparing healthy tissue. Ionizing radiation (photons and ions) causes damage throughout the cell. However the most susceptible part to radiation is the
...
radiotherapy strives to avoid.


\subsubsection{Relative Biological Effectiveness}
\label{RBE}

Fig.~\ref{track} shows the size of DNA molecule in comparison with proton and carbon ion distribution of $\delta$-electrons around their track. A clustered DSB occurs preferably around Bragg peak due to large ionization densities. As mentioned
clustered DSB pose a too complex task for cell repair-mechanisms to handle, leading eventually to cell death. Ions have large ionization density and is one of the main advantages over photons in the radiotherapeutic sense. Since most of the clinical experience about
...
At GSI, RBE is calculated using the \textit{local effect model} (LEM) developed by Scholz et al. \cite{Scholz1994}. The main assumption of the LEM model is that localized biological 
...
\cite{Mitaroff1998, Kraemer2000a, Kraemer2003} and it was used in clinic. RBE for carbon ions ranges from 1 in the entrance channel, to a value around 5 at 
...
However, clinical results show othewise with good tumor local control, which indicates different mechanisms at work. The topic is still under research and no consensus has yet been reached regarding the theory behind it.
...
A photon therapy directs high-energy (MV) photons to target. To achieve that, electrons are first accelerated in linear accelerators. Next, they are stopped in a heavy metal target where photons are produced via bremsstrahlung.
...
where a variable number of beams is used. Each beam is then shaped with a multileaf collimator. Even more precise technique is intensity-modulated radiotherapy (IMRT). IMRT allows treating complex tumor shapes, e.g. when the tumor is in proximity of a critical structure.
...
onto the target with high accuracy (around 1 mm). Both, complex delivery system and the cost of the accelerator make ion radiotherapy not as common as photon radiotherapy.
...
\caption{A picture of a multileaf collimator. Individual leaves are positioned so that exit beam from linear accelerator conforms to the patient tumor in a beam's eye view. Picture taken from \cite{MLC}.}
\label{MLC}
\end{center}
\end{figure}
...
\newline
\textbf{Intrafractional motion} is mainly caused by respiration and heart beat, but also pristalsis. The time scale ranges from seconds to minutes. In this thesis we investigated treatment of lung cancer, so focus lies on respiratory motion. Respiratory motion varies from patient to patient and 
...
The solution for longitudinal changes was carried out by two polymethyl methacrylate (PMMA) wedges close to the target, that are operated via linear stepmotor \cite{Saito2009}, as shown in Fig.~\ref{tracking}. The stepmotor can
change the relative distance between the wedges and therefore introduces more or less material the beam travels through and consequently changes the effective beam energy (range). The beam tracking system at GSI is able to achieve high precision \cite{Bert2007, Bert2009, Saito2009}. Clinical implementation, however, is not yet feasible due to several reasons, i. e. the precision and speed of motion monitoring, complexity of ion-beam tracking system, inverse interplay.
