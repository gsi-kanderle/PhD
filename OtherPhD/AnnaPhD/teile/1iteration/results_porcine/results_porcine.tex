...
%  \includegraphics[scale=0.4]{L829_PVsandAV.png}
 \includegraphics[scale=0.3]{L829_PVsandAV.png}
 }
\subfigure[PVs and CTI]{
%  \includegraphics[scale=0.4]{L829_PVsandCTI.png}
 \includegraphics[scale=0.3]{L829_PVsandCTI.png}
 }
...
% % beam direction
% % 
% % \subsubsection{Dose to organs at risk}
% % 
...
% % 
% % 
% % \subsubsection{Motion mitigation techniques for dose deposition in AV node}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% In table \ref{tab:motion_native_vs_contrast} a numerical comparison between the motion obtained from native and contrast enhanced CT scans is shown. 
...
% The motion information for all pigs in case of contrast CTs can be found in section \ref{Sec:Motion_pigs}.
...
% % The mean and standard deviation over all motion phases are stated for each pig in 
% % table \ref{tab:motion_allpigs}.
% % Depending on the studied target volume, the highest absolute displacement can be observed in different pigs. 
% % While the PVs and CTI move the most in pig 1 with a displacement of up to 3mm and 4mm, respectively, the maximal AV node movement is 
% % observed in pig 2 (up to 4mm). Independent of the studied target volume the largest contribution to the absolute displacement is found in 
% % AP direction for all pigs. This can also be seen in the mean displacements over all pigs. 
% % For the PVs the mean amplitude in SI direction is (0.30 $\pm$ 0.77)mm, (1.53 $\pm$ 1.06) in AP direction and (-0.46 $\pm$ 0.95) in LR 
% % direction. In case of the CTI the mean amplitude in SI is (0.75 $\pm$ 0.74), (1.84 $\pm$ 0.65) in AP and (0.52 $\pm$ 0.85) in LR. For the AV 
% % node it is (0.71 $\pm$ 0.33) in SI, (2.42 $\pm$ 0.36) in AP and (-0.67  $\pm$ 0.47) in LR. On average, the absolute amplitude over all motion 
% % phases and pigs is found to (2.32 $\pm$ 0.79)mm for the PVs, (2.78 $\pm$ 0.68) for CTI and (2.97 $\pm$ 0.39)mm for the AV node.
% % Concerning the maximal absolute displacement it can be seen in table \ref{tab:maxabsmotion_allpigs} that the results vary not only for the 
% % cardiac target volumes but are also dependent on the pig data set. Furthermore it can be seen that in the maximal absolute displacement the 
% % CTI moves with a bigger amplitude than the other two target volumes, in three of the four studied pigs (difference found in pig 3). 
% % The smallest displacement is found in the PVs, again in three of the four studied data sets (again the difference is found in pig 3). 
% % This can be interpreted in connection to the placement of the target volumes within the heart. While the PVs are found in the upper atria, 
% % the CTI is located in the lower atria and hence the influence of the ventricular motion should be bigger. The AV node is in between 
% % the atria and ventricles and was found to have an intermediate absolute displacement in most cases (exception in pig 3).\newline
% % \newline
% % Furthermore it can be seen in table \ref{tab:maxabsmotion_allpigs} that no motion phase can be directly connected to the maximal absolute 
% % displacment of the target volumes. Since the motion phases of the heartbeat gated CT scan are based on the ECG trace and result in a division 
% % of a single heartbeat the motion phases should be directly assigned to the contraction (systole) and dilatation (diastole) of both atria and 
% % ventricles. Contraction of the atria (atrial systole) is occurring in between motion phase four and nineteen, in the same time as the 
% % ventricular relaxation (ventricular diastole). The ventricular systole and at the same time atria diastole are hence much shorter, occurring 
% % in the remaining motion phases twenty to three. 
% % % The maximal displacement of the atria should thus be observed in motion phase eightteen, while 
% % % the maximal amplitude of the ventricle should be observed in motion phase three. 
% % Looking into more detail into the motion pattern of the target volumes based on the motion phase division, it can be seen that the 
% % CTI moves most rapidly between motion phase 14 and 19, and between motion phase 1 and 5. 
% % 
% % % While no motion phase can be connected to a maximal displacment for the different cardiac target volumes in all pig cases, the motion can be 
% % % interpreted in connection to the placement of the target volumes within the heart.  
% % % The CTI, which is found on the lower atrium, moves most rapidly between motion phase 15 and 19, and between motion phase 1 and 6. 
% % % 
% % %
% % % The motion of the PVs on the other hand result in a much more 
...
% % % 
% % % For none of the studied porcine data sets a maximal motion phase 
% % 
% % % \newpage
...
\begin{figure}[H]
\centering
\subfigure[Pig 2: max absolute motion (MP 11)]{
%  \includegraphics[scale=0.21]{Contour_z_abs_HB_L833_11_gedreht.png}
 \includegraphics[scale=0.16]{Contour_z_abs_HB_L833_11_gedreht.png}
}
\subfigure[Pig 4: max absolute motion (MP 10)]{
% \includegraphics[scale=0.21]{Contour_z_abs_HB_L979_10_gedreht.png}
\includegraphics[scale=0.16]{Contour_z_abs_HB_L979_10_gedreht.png}
}
...
% % 
% % 
% % - BILD dy von L833: sagittal view -> man sieht das konturen am herzem zwischen native und contrast ok!
% % - large difference between native and contrast! naturally ecsp. for targets inside of heart (AV and CTI) \newline 
...
% % 
% % - possible solution: vectorfield information from contrast ct on native ct!!!!
% % 
% % \newpage
...
\begin{figure}[H]
\subfigure[Esophagus]{
 \includegraphics[scale=0.18]{Mayo_Pigs_ESO_alternativeLimit.png}
 }
 \subfigure[Trachea]{
 \includegraphics[scale=0.18]{Mayo_Pigs_TRACHEA.png}
 }
  \subfigure[Heart]{
 \includegraphics[scale=0.18]{Mayo_Pigs_HEARTwoOverlap.png}
 }
  \subfigure[Aorta]{
 \includegraphics[scale=0.18]{Mayo_Pigs_AORTA.png}
 }
...
% LV: median mean dose 0.5Gy (0.5Gy), median max dose 26.2Gy (26.3Gy), max volume 5.6\% (6.4\%)
% RV: median mean dose 0.4Gy (0.5Gy), median max dose 25.2Gy (25.5Gy), max volume 9.3\%(11.9 \%)
% 
% LCA: median mean dose 0.6Gy (0.7Gy), median max dose 7.3Gy (7.4Gy), max volume 14.1\% (17.2\%)
% RCA: median mean dose 1.4Gy (1.4Gy), median max dose 9.4Gy (9.7Gy), max volume 35.7\% (37.5\%)


% 
% \begin{table}[htbp]
% \centering
% \caption{Median as well as maximum point dose to the whole heart and different critcal cardiac substructures as well as maximal irradiated 
% volume of the structures for all pigs.}
%   \begin{tabular}{ |c||c||c|c|c|}
%     \hline
%     OAR & Pig & Mean dose [Gy] & Max dose [Gy] & Max volume [\%] \\ \hline \hline
% Heart & 1 & 0.57 & 25.77 & 13.95 \\ \hline 
% Heart & 2 & 0.51 & 25.42 & 12.74 \\ \hline
% Heart & 3 & 0.76 & 25.27 & 17.44 \\ \hline
% Heart & 4 & 0.77 & 25.72 & 16.57 \\ \hline
% \hline
% LA & 1 & 0.86 & 25.87 & 28.01 \\ \hline
% LA & 2 & 0.93 & 25.25 & 25.53 \\ \hline
% LA & 3 & 1.37 & 25.72 & 31.36 \\ \hline
% LA & 4 & 2.05 & 26.00 & 41.70 \\ \hline
% \hline
% RA & 1 & 1.87 & 26.20 & 35.76 \\ \hline
% RA & 2 & 1.46 & 26.15 & 23.82 \\ \hline
% RA & 3 & 1.44 & 25.62 & 35.41 \\ \hline
% RA & 4 & 2.25 & 26.10 & 24.85 \\ \hline
% \hline
% LV & 1 & 0.53 & 26.30 & 6.67 \\ \hline 
% LV & 2 & 0.52 & 26.25 & 6.12 \\ \hline
% LV & 3 & 0.33 & 26.07 & 5.06 \\ \hline
% LV & 4 & 0.40 & 25.97 & 4.51 \\ \hline
% \hline
% RV & 1 & 0.26 & 25.15 & 9.06 \\ \hline
% RV & 2 & 0.23 & 25.15 & 7.95 \\ \hline
% RV & 3 & 0.44 & 25.25 & 9.56 \\ \hline
% RV & 4 & 0.52 & 25.77 & 14.28 \\ \hline
% \hline
% LCA & 1 & 0.82 & 7.37 & 19.86 \\ \hline
% LCA & 2 & 0.60 & 7.15 & 13.53 \\ \hline
% LCA & 3 & 0.24 & 6.42 & 8.91 \\ \hline
% LCA & 4 & 0.66 & 7.42 & 14.61 \\ \hline
% \hline
% RCA & 1 & 1.49 & 9.15 & 37.59 \\ \hline
% RCA & 2 & 1.12 & 8.42 & 23.98 \\ \hline
% RCA & 3 & 1.35 & 9.75 & 37.31 \\ \hline
% RCA & 4 & 1.39 & 9.62 & 34.09 \\ \hline
% \hline
%     \hline
%   \end{tabular}
%   \label{DV_OAR}
...
% % \begin{table}[htbp]
% % \centering
% % \caption{Mean and standard deviation as well as maximum point dose to the whole heart and different critcal cardiac substructures for all 
% %   four pigs.}
% %   \begin{tabular}{ |c||c||c|c|}
% %     \hline
% %     OAR & Pig & Dose: Mean $\pm$ Stdev [Gy] & Dose: Max [Gy]\\ \hline \hline
% % Heart & 1 & 0.57 $\pm$ 2.33 & 25.77 \\ \hline
% % Heart & 2 & 0.51 $\pm$ 2.21 & 25.42 \\ \hline
% % Heart & 3 & 0.76 $\pm$ 2.90 & 25.27 \\ \hline
% % Heart & 4 & 0.77$\pm$ 2.80 & 25.72 \\ \hline
% % \hline
% % LA & 1 & 0.86 $\pm$ 2.70 & 25.87 \\ \hline
% % LA & 2 & 0.93 $\pm$ 2.65 & 25.25 \\ \hline
% % LA & 3 & 1.37 $\pm$ 3.55 & 25.72 \\ \hline
% % LA & 4 & 2.05 $\pm$ 4.60 & 26.00 \\ \hline
% % \hline
% % RA & 1 & 1.87 $\pm$ 5.25 & 26.20 \\ \hline
% % RA & 2 & 1.46 $\pm$ 4.75 & 26.15 \\ \hline
% % RA & 3 & 1.44 $\pm$ 4.47 & 25.62 \\ \hline
% % RA & 4 & 2.25 $\pm$ 5.92 & 26.10 \\ \hline
% % \hline
% % LV & 1 & 0.53 $\pm$ 3.12 & 26.30 \\ \hline 
% % LV & 2 & 0.52 $\pm$ 3.15 & 26.25 \\ \hline
% % LV & 3 & 0.33 $\pm$ 2.46 & 26.07 \\ \hline
% % LV & 4 & 0.40 $\pm$ 2.82 & 25.97 \\ \hline
% % \hline
% % RV & 1 & 0.26 $\pm$ 1.63 & 25.15 \\ \hline
% % RV & 2 & 0.23 $\pm$ 1.54 & 25.15 \\ \hline
% % RV & 3 & 0.44 $\pm$ 2.55 & 25.25 \\ \hline
% % RV & 4 & 0.52 $\pm$ 2.44 & 25.77 \\ \hline
% % \hline
% % LCA & 1 & 0.82 $\pm$ 2.06 & 7.37 \\ \hline
% % LCA & 2 & 0.60 $\pm$ 1.84 & 7.15 \\ \hline
% % LCA & 3 & 0.24 $\pm$ 0.99 & 6.42 \\ \hline
% % LCA & 4 & 0.66 $\pm$ 1.93 & 7.42 \\ \hline
% % \hline
% % RCA & 1 & 1.49 $\pm$ 2.85 & 9.15 \\ \hline
% % RCA & 2 & 1.12 $\pm$ 2.55 & 8.42 \\ \hline
% % RCA & 3 & 1.35 $\pm$ 2.77 & 9.75 \\ \hline
% % RCA & 4 & 1.39 $\pm$ 2.95 & 9.62 \\ \hline
% % \hline
% %     \hline
% %   \end{tabular}
% %   \label{DV_OAR}
...
% Exemplary the mean values for dose coverage are stated for all motions in table \ref{tab:meandoseparameters_allpigs_interplay}. 
\newline
...
% % % Exemplary, the results of pig 2 (with the largest absolute displacement of the AV node) 
% % % and pig 4 (with the smallest absolute displacement of the AV node) will be discussed. 
% % % The dose coverage for a static irradiation of the AV node with 5mm margin is found to 100\% in this pig. This value decreases to 91.2\% for an 
% % % uncompensated motion with 0.5s period and a starting phase of 0$^{\circ}$. With rescanning the result can be improved to 100\% with ten 
% % % rescans or 99.5\% with fifteen rescans. The over dosage is found to 9.3\% with ten rescans for the stated motion case, while it can be 
% % % reduced to 0\% (static: 0\%) with fifteen rescans. The dose steepness on the other hand is slighlty better with ten rescans compared to fifteen, 
% % % resulting to 6.2\% versus 7.3\% for the presented motion case (compared to 4.4\% in the static case). For pig 4 on the other hand, which is 
% % % the data set with the smallest absolute displacement of the AV node, it can be observed that e.g. the dose coverage is improved with fifteen 
% % % rescans (100\%) compared to ten rescans (97.8\%) for a motion of 0.7s period and starting phase of 90$^{\circ}$. \newline
\newline
...
% It can hence be concluded that 
% ten rescans already yield good results in all studied motion cases, but nevertheless the result needs to be checked individually for each 
% porcine data set and eventually increased to a slighlty higher rescan number. 
...
\vspace*{0.4cm}
 ...
 \centering
\subfigure[static]{
 \includegraphics[scale=0.32]{Mayo_Pig_L833_contrast_STATIC_withLegend.png}
 }
\subfigure[interplay]{
 \includegraphics[scale=0.32]{Mayo_Pig_L833_contrast_INTERPLAY_sin05s90_withLegend.png}
 }
 \subfigure[rescanning (15x)]{
 \includegraphics[scale=0.32]{Mayo_Pig_L833_contrast_RESCANNING_15x_sin05s90_withLegend.png}
 }
...
% \begin{table}[H]
%   \centering
%   \caption{Interplay: Mean value and standard deviation of dose parameters over all pigs for irradiation of the AV node with a safety margin of 5mm.}
%   \begin{tabular}{|c|c||c|c|c|}
%     \hline\hline
%     Motion period [s] & Motion phase [$^{\circ}$] & D5-D95 [\%] & V95 [\%] & V107 [\%] \\
%     \hline
%     0.7 & 0 & 10.2 $\pm$ 3.8 & 84.3 $\pm$ 17.5 & 13.0 $\pm$ 22.5 \\
%     0.7 & 90 & & & \\
%     0.5 & 0 & 13.0 $\pm$ 4.0 & 84.5 $\pm$ 8.0 & 5.5 $\pm$ 7.0 \\
%     0.5 & 90 & & & \\
%     \hline\hline
%     \end{tabular}
%   \label{tab:meandoseparameters_allpigs_interplay}
% \end{table}
% 
% \begin{table}[H]
%   \centering
%   \caption{Interplay: Mean value and standard deviation of dose coverage over all pigs for irradiation of the AV node with a safety margin of 5mm.}
%   \begin{tabular}{|c|c||c|}
%     \hline\hline
%     Motion period [s] & Motion phase [$^{\circ}$] & V95 [\%] \\
%     \hline
%     0.7 & 0 & 84.3 $\pm$ 17.5 \\
%     0.7 & 90 & 91.0 $\pm$ 10.3 \\
%     0.5 & 0 & 84.5 $\pm$ 8.0 \\
...
% While the coronary atrial circulation is very similar to humans \cite{Wea86} th venous circulation is different from humans. 
\newline
...
% As was already stated in the comparison of cardiac anatomy the location of the heart as well as the thorax form 
% differ between porcine and humans. It is thus not suprising these finding vary from the results of heartbeat induced motion in human 
% hearts (see chapter XXX), where no dominant motion direction was found. 
...
% - motion very dependent on pig - hence carefull consideration of motion in individual treatment plans for each pig 
% - contrast CT needed: possible solution: use vectorfield of contrast CT on nativ CT 
% - comparison to human hb motion 
...
% % Limin Abstract:
% % - same porcine data (?)
% % - comparison IMRT (photons) and p-plan (single ap field)
% % - 5-6 mm margin
% % - AV node irradiation 
% % - single p field: 30 Gy
% % - average mean dose to AV node: IMRT: 32 pm 0.6 Gy, p: 30.7 pm 0.2 Gy
% % - mean overall cardiac dose: IMRT: 4.7 pm 1.3 Gy, p: 3.6 pm 1.5 Gy
% % ----- IMRT:
% % - average max dose to spinal cord: 2.9 Gy
% % - mean dose to eso: 1.6 Gy
% % - mean dose trachea: 2.2 Gy
% % - mean dose lung: 1 Gy
% % ------ p:
% % - negliggible dose exposure to these risk structures
% % 
% % 
% % - RTOG protocol: for human, no idea if pigs more or less dose to critical structures 
% % 
% % 
% % 
...
% - wie bei anderen zentren herzbewegung in schweinen kompensiert, generell experimten (auch in introduction) 
